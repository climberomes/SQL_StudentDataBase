...
#Inner join to have only complete tables linked
#Search only for courses
#HAVING is important here to filter out searches
echo -e "\nList of courses, in alphabetical order, with only one student enrolled:"
echo "$($PSQL "SELECT course FROM students INNER JOIN majors_courses USING(major_id) INNER JOIN courses USING(course_id) GROUP BY course HAVING COUNT(courses)=1 ORDER BY course")"
